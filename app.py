...
st.markdown("""
音声ファイル（MP3, WAV, M4A, MP4）をアップロードするだけで、Gemini_2.5_proが自動で文字起こしを行い、議事録を作成します。
""")
...
if uploaded_file is not None:
    ...
    with st.spinner("ステップ3/5: AIが音声を文字に変換しています...（この処理は数分かかることがあります）"):
     ...
     この音声ファイルの全ての音声を以下の条件にしたがって、文字起こしして。

     【条件】
     ・感情、感嘆、不要な言葉、フィラーを削除する
     ・【用語】、【事前情報】の単語が頻繁に登場します。表記は必ず【用語】、【事前情報】の表記に統一する
     ・各発話ごとに必ず対応する音声の開始時間（タイムスタンプ）を正確に記入してください。
     ・タイムスタンプは [hh:mm:ss] の形式で、音声の実際の時間に厳密に一致するようにしてください。
     ・音声と文字の対応がずれないように、細心の注意を払ってください。
    ...
    with st.spinner("ステップ4/5: AIが文字起こし内容を要約し、議事録を作成しています..."):
        ...
        文字起こししたテキストに誤字があるため誤字を修正し、以下のフォーマットを厳密に守って、日本語で出力してください。
        （）内の文字は指示文のため出力しないでください。
        ...
        ----------------------------------------------------------------------------------------------------------
        ## 2. 案件進捗確認
        （会議で具体的に決定、共有された事項を、箇条書きでリストアップしてください。決定事項がない場合は「特になし」と記載してください。）
        ■Ａ（太字）
        ・Aについての詳細
        ・Aについての詳細
        ■Ｂ（太字）
        ...
        ----------------------------------------------------------------------------------------------------------
        ## 3. スケジュール確認
       （会議内で決定、共有されたスケジュールを箇条書きする）

       （その他の必要事項を４．５．と付け加える。必要なければ記入する必要はない）
        ---

...
if uploaded_file is not None:
    ...
    with st.spinner("ステップ3/5: AIが音声を文字に変換しています...（この処理は数分かかることがあります）"):
     ...
     ・文字起こしした文が対応する音声ファイルの時間を記入する
     ・話者はspeaker1などと表記し、人名は特定しない
     ・「speaker1, 00:00:47,よろしくお願いいたします。」のように表記を統一する
     ・一度文字起こししたテキストの誤字を【用語】、【事前情報】を踏まえ修正する。
     ・一度文字起こししたテキストの時間表記の間違いを修正する
    ...
    with st.spinner("ステップ4/5: AIが文字起こし内容を要約し、議事録を作成しています..."):
        ...
        以下のフォーマットを厳密に守って、日本語で出力してください。ただし、文字起こししたテキストに誤字がある可能性を考慮する。

        # 議事録
        ## 1. この会議の概要
        （録音時間から会議の時間を推測してください）
        会議時間：録音時間より約〇分程度
        （会議内容から議題を抽出してください）
        議題：
        -------------------------------------------------------------------
